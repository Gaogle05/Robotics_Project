...
class PID(object):
    ...
    def compute_pid(self, error):
        ...
        self.derivative_error = self.error - \
            self.error_last  # derivative is error-prev error
        self.error_last = self.error  # update previous error
        self.output = self.kp*self.error + self.ki*self.integral_error + \
            self.kd*self.derivative_error  # calculate the PID value
        return self.output
    ...
    def compute_pid_angular(self, yaw):
        ...
        self.output = self.kp*self.error + self.ki*self.integral_error + \
            self.kd*self.derivative_error  # calc PID
        return self.output
